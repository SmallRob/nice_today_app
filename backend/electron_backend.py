...
class ElectronBackendService:
    ...
    def get_today_biorhythm(self, birth_date):
        ...
        try:
            ...
            return self.get_date_biorhythm(birth_date, today)
    ...
    def get_biorhythm_range(self, birth_date, days_before, days_after):
        ...
        try:
            ...
            while current_date <= end_date:
                result = self.get_date_biorhythm(birth_date.strftime("%Y-%m-%d"), current_date.strftime("%Y-%m-%d"))
                if result['success']:
    ...
    def get_today_maya_info(self):
        ...
        try:
            today = datetime.now().strftime("%Y-%m-%d")
            return self.get_date_maya_info(today)
        except Exception as e:
    ...
    def get_maya_info_range(self, days_before, days_after):
        ...
        try:
            ...
            while current_date <= end_date:
                result = self.get_date_maya_info(current_date.strftime("%Y-%m-%d"))
                if result['success']:
                    results.append(result['data'])
                current_date += timedelta(days=1)
    ...
    def get_today_dress_info(self):
        ...
        try:
            today = datetime.now().strftime("%Y-%m-%d")
            return self.get_date_dress_info(today)
        except Exception as e:
    ...
    def get_dress_info_range(self, days_before, days_after):
        ...
        try:
            ...
            while current_date <= end_date:
                result = self.get_date_dress_info(current_date.strftime("%Y-%m-%d"))
                if result['success']:
                    results.append(result['data'])
                current_date += timedelta(days=1)
...
def handle_method_call(method_name, args_json):
    """处理方法调用"""
    # 添加调试信息
    print(f"DEBUG: method_name={method_name}", file=sys.stderr)
    print(f"DEBUG: args_json={repr(args_json)}", file=sys.stderr)
    
    try:
        # 解析参数
        args = json.loads(args_json) if args_json else {}
    except json.JSONDecodeError as e:
        print(json.dumps({"success": False, "error": f"参数解析失败: {str(e)}, 原始参数: {repr(args_json)}"}))
        return
    ...
    try:
        if method_name == 'get_today_biorhythm':
            result = service.get_today_biorhythm(args.get('birth_date'))
        elif method_name == 'get_date_biorhythm':
            result = service.get_date_biorhythm(args.get('birth_date'), args.get('target_date'))
        elif method_name == 'get_biorhythm_range':
            result = service.get_biorhythm_range(args.get('birth_date'), args.get('days_before', 7), args.get('days_after', 7))
        elif method_name == 'get_today_maya_info':
            result = service.get_today_maya_info()
        elif method_name == 'get_date_maya_info':
            result = service.get_date_maya_info(args.get('target_date'))
        elif method_name == 'get_maya_info_range':
            result = service.get_maya_info_range(args.get('days_before', 7), args.get('days_after', 7))
        elif method_name == 'get_maya_birth_info':
            result = service.get_maya_birth_info(args.get('birth_date'))
        elif method_name == 'get_today_dress_info':
            result = service.get_today_dress_info()
        elif method_name == 'get_date_dress_info':
            result = service.get_date_dress_info(args.get('target_date'))
        elif method_name == 'get_dress_info_range':
            result = service.get_dress_info_range(args.get('days_before', 7), args.get('days_after', 7))
        else:
