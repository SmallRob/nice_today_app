...
function buildFrontend() {
    console.log('\n📦 构建女性健康管理前端应用...');
    try {
        // 设置环境变量以使用女性健康管理的入口文件
        const env = Object.assign({}, process.env, {
            ENTRY_FILE: 'src/women-health-index.js'
        });
...
function copyFrontendToElectron() {
    ...
    const electronPublicDir = 'electron/public/women-health';
...
function buildElectron() {
    ...
    try {
        // 根据是否为开发模式选择不同的配置文件
        const configFlag = isDev ? '--config electron-builder-local.json' : '--config electron-builder-women-health.json';
        const buildCommand = `cd electron && npx electron-builder ${configFlag}`;
...
async function main() {
    try {
        ...
        if (testBuildResult()) {
            ...
            console.log('\n📖 使用说明:');
            console.log('  • 构建生产版本: npm run build:women-health');
            console.log('  • 构建开发版本: npm run build:women-health -- --dev');
        } else {
