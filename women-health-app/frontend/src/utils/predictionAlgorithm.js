...
export class PredictionEngine {
   ...
   * 基于历史数据预测下一个周期
   */
  static predictNextCycle(history) {
   ...
   * 计算周期统计数据
   */
  static calculateCycleStatistics(history) {
    if (history.length === 0) {
      return {
   ...
   * 计算加权平均值（最近的数据权重更高）
   */
  static calculateWeightedAverage(values) {
   ...
   * 计算周期规律性
   */
  static calculateCycleRegularity(cycleLengths) {
   ...
   * 计算预测置信度
   */
  static calculatePredictionConfidence(history) {
   ...
   * 生成日历事件
   */
  static generateCalendarEvents(prediction) {
    const events = [
      {
